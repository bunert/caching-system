...
import simulation.helper.request as request
import simulation.helper.addr as addr
import simulation.helper.redisLog as redisLog
import common
...
def setupArgs():
    ...
    parser.add_argument("-l", "--localhost", dest="localhost", action='store_true',
        help="if running on our system, specify if running on localhost") # flag with default False
...
if __name__ == '__main__':
    ...
    # our system:
        ...
        # get gateway addr if running on EC2 and localhost otherwise
        if (args.localhost):
            logging.info("system running on localhost")
            request.gateway_addr = "http://localhost:4000/api/v1/objects/"
        else:
            logging.info("system running on EC2")
    ...
    output_filename, output_filepath = common.simulate_check_output(args.name, args.filename, system)

    results, start, end = request.start(args.filename)

    if args.system == "system":
        if (args.localhost):
            logging.info("get EC2-redis log file from localhost")
            redisLog.getLogFileLocal(args.name, output_filename)
        else:
            logging.info("get EC2-redis log file from Orchestrator")
            redisLog.getLogFileRemote(args.name, output_filename, orchestrator_addr)
