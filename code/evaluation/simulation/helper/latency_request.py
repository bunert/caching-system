...
class Request: 
    ...
    def execute(self, error_event):
        ...
        # latency in ms
        diff = round(r.elapsed.total_seconds()*1000, 3)
        logging.info(f"executed request (id: {self.id:>3}, delay: {self.delay:>6}, time until header: {diff:>6}ms): {latency:>6}ms")
...
def setup_jobs(filename, q, object):
    filepath = os.path.join(os.getcwd(), filename)
    with open(filepath, 'r') as f:
        reader = csv.reader(f, delimiter='\t')
        for row in reader:
            q.put(Request(int(row[0]), float(row[1]), row[2], object))
